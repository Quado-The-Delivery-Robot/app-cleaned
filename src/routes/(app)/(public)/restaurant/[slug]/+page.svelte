...
<div class="flex flex-col gap-6 justify-start items-start">
    {#await data.restaurant && data.feed}
        <p>Loading...</p>
    {:then}
        <div class="flex gap-0.5 text-lg relative">
            {#each Array(2) as _, index}
                <div class="rating flex gap-0.5 bg-clip-content {index == 1 ? 'absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' : ''}" style="width: {index == 1 ? (index / 5) * 100 : 100}%;">
                    {#each Array(5) as _index}
                        <svg class="w-6 aspect-square {index == 1 ? 'fill-yellow-500' : 'fill-white'}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
