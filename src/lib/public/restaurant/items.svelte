...
<div class="w-full flex flex-col items-start justify-start lg:flex-row gap-4">
    <div class="flex gap-2 flex-row lg:flex-col w-full lg:min-w-[188px] lg:w-[188px] h-fit overflow-x-auto">
        {#each Object.keys(groups) as group}
            <button
                class="w-fit lg:w-full px-4 lg:px-3 py-2 lg:py-3 text-nowrap text-left hover:bg-secondary border-opacity-0 hover:border-opacity-100 border border-secondary-300 transition-all rounded-lg"
                on:click={() => {
                    currentGroup = group;
                }}>{group}</button
            >
    ...
    <div class="flex-grow grid grid-cols-1 lg:grid-cols-2 auto-rows-max gap-3">
        {#each items as item}
            {#if item.type === currentGroup || currentGroup === "All"}
                <div class="primary w-full overflow-hidden rounded-lg">
                    <div class="w-full flex flex-col lg:flex-row justify-start items-center gap-2 overflow-hidden pr-6 h-full">
                        ...
                        <img class="w-28 lg:w-36 aspect-square" src={item.image} alt={item.name} />
                    </div>
                </div>
            {/if}
        {/each}
